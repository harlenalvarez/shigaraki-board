...
import { clearCanvas, getClickedPoint, getConnectionPoints, getMidPoint, getSlope, nodeArcAutoPosition, NodeSection } from '@/utils';
...
export const SphereHandler = () => {
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    const sphere = new Node2D({ radius: pointRadius, point: { x: currentX, y: currentY }, color: '#ccc' });
    const nodeText = new Text2D({ value: 'Simplify', point: { x: currentX, y: currentY }, color: '#333943', alignment: 'center', baseline: 'middle' });
    ...
    for (let index = 0; index < 10; index++) {
      ...
      const sphere = new Node2D({ radius: level1NodeRadius, point: { x, y }, color: '#ccc' });
      const nodeText = new Text2D({ value: `${index + 1}`, point: { x, y }, color: '333943', alignment: 'center', baseline: 'middle' });
      renderedObjectsIntance.push(sphere);
      renderedText.push(nodeText);
    }
